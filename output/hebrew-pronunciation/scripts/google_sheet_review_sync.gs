...
function rowToObject_(row) {
  return {
    ...
    flow_issue: row[9],
    flow_action: row[10],
    flow_start_line: row[11],
    flow_end_line: row[12],
    flow_note: row[13],
    page_signed_off: row[14],
    page_signed_off_at: row[15],
...
function objectToRow_(record) {
  return [
    ...
    record.flow_issue ? true : '',
    record.flow_action || '',
    record.flow_start_line === null || typeof record.flow_start_line === 'undefined' ? '' : record.flow_start_line,
    record.flow_end_line === null || typeof record.flow_end_line === 'undefined' ? '' : record.flow_end_line,
    record.flow_note || '',
...
function loadReviewState_() {
  ...
  rows.forEach(function (row) {
    ...
    lineReviews.push({
      ...
      flowIssue: Boolean(row.flow_issue),
      flowAction: row.flow_action || '',
      flowStartLine: row.flow_start_line === '' ? null : Number(row.flow_start_line),
      flowEndLine: row.flow_end_line === '' ? null : Number(row.flow_end_line),
      flowNote: row.flow_note || '',
    });
  });
...
function savePageReview_(payload) {
  ...
  const lineRecords = (payload.lineReviews || []).map(function (review) {
    return {
      ...
      flow_issue: review.flowIssue ? true : '',
      flow_action: review.flowAction || '',
      flow_start_line: typeof review.flowStartLine === 'number' ? review.flowStartLine : '',
      flow_end_line: typeof review.flowEndLine === 'number' ? review.flowEndLine : '',
      flow_note: review.flowNote || '',
